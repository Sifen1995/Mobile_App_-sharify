...
fun SignInScreen(
    ...
    if (uiState.success) {
        LaunchedEffect(Unit) { // 👈 Runs only ONCE after login success
            ...
            when (uiState.userRole) {
                ...
                else -> navController.navigate(Screen.UserHome.route) // Default fallback
            }

            viewModel.resetSuccess() // ✅ Ensure success state is cleared
        }
    }
